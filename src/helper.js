  ...
  const iter = (obj1, obj2, depth = 1) => {
    ...
    return sortedKeys.reduce((acc, key) => {
      ...
      if (_.isObject(val1) && _.isObject(val2)) {
        ...
        return { ...acc, [`${spaceSymbol.repeat((leftShift) * indentSize)}${key}`]: nestedDiff };
      }
      if (!Object.hasOwn(obj1, key)) {
        return { ...acc, [`${spaceSymbol.repeat((leftShift) * indentSize)}${'+'} ${key}`]: val2 };
      }
      if (!Object.hasOwn(obj2, key)) {
        return { ...acc, [`${spaceSymbol.repeat((leftShift) * indentSize)}${'-'} ${key}`]: val1 };
      }
      if (!_.isEqual(val1, val2)) {
        return {
          ...acc,
          [`${spaceSymbol.repeat((leftShift) * indentSize)}${'-'} ${key}`]: val1,
          [`${spaceSymbol.repeat((leftShift) * indentSize)}${'+'} ${key}`]: val2,
        };
      }
      return acc;
    }, {});
